window.addEventListener("load", function () {
  const boxes = document.querySelectorAll(".special-readdd-boxes-list>div");
  ...
  const viewBtn = document.querySelectorAll(".vdocc-view");
  const splinner = document.querySelector(".vdocc-book-spinn");
  const pdfPop = document.querySelector(".vdocc-book-pop");
  const iframePop = document.querySelector(".vdocc-book-pop iframe");
  const iframePopBody = document.querySelector(".vdocc-book-pop>div>div>div");
  boxes.forEach((item, idx) => {
    let dataLink = item.getAttribute("data-link");
