window.addEventListener("load", function () {
  const boxes = document.querySelectorAll(".wasserrr-boxes-list>div");
  ...
  const viewBtn = document.querySelectorAll(".wasserrre-view");
  const splinner = document.querySelector(".wasserrre-book-spinn");
  const pdfPop = document.querySelector(".wasserrre-book-pop");
  const iframePop = document.querySelector(".wasserrre-book-pop iframe");
  const iframePopBody = document.querySelector(".wasserrre-book-pop>div>div>div");
  boxes.forEach((item, idx) => {
    let dataLink = item.getAttribute("data-link");
