window.addEventListener("load", function () {
  const boxes = document.querySelectorAll(".sehrrrr-boxes-list>div");
  ...
  const viewBtn = document.querySelectorAll(".sehrrrre-view");
  const splinner = document.querySelector(".sehrrrre-book-spinn");
  const pdfPop = document.querySelector(".sehrrrre-book-pop");
  const iframePop = document.querySelector(".sehrrrre-book-pop iframe");
  const iframePopBody = document.querySelector(".sehrrrre-book-pop>div>div>div");
  boxes.forEach((item, idx) => {
    let dataLink = item.getAttribute("data-link");
