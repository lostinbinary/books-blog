window.addEventListener("load", function () {
  const boxes = document.querySelectorAll(".grossss-boxes-list>div");
  ...
  const viewBtn = document.querySelectorAll(".grosssse-view");
  const splinner = document.querySelector(".grosssse-book-spinn");
  const pdfPop = document.querySelector(".grosssse-book-pop");
  const iframePop = document.querySelector(".grosssse-book-pop iframe");
  const iframePopBody = document.querySelector(".grosssse-book-pop>div>div>div");
  boxes.forEach((item, idx) => {
    let dataLink = item.getAttribute("data-link");
