window.addEventListener("load", function () {
  const boxes = document.querySelectorAll(".weinnn-boxes-list>div");
  ...
  const viewBtn = document.querySelectorAll(".weinnne-view");
  const splinner = document.querySelector(".weinnne-book-spinn");
  const pdfPop = document.querySelector(".weinnne-book-pop");
  const iframePop = document.querySelector(".weinnne-book-pop iframe");
  const iframePopBody = document.querySelector(".weinnne-book-pop>div>div>div");
  boxes.forEach((item, idx) => {
    let dataLink = item.getAttribute("data-link");
