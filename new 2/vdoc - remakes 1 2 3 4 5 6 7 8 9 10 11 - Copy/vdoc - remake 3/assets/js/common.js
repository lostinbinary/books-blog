window.addEventListener("load", function () {
  const boxes = document.querySelectorAll(".tschusss-boxes-list>div");
  ...
  const viewBtn = document.querySelectorAll(".tschussse-view");
  const splinner = document.querySelector(".tschussse-book-spinn");
  const pdfPop = document.querySelector(".tschussse-book-pop");
  const iframePop = document.querySelector(".tschussse-book-pop iframe");
  const iframePopBody = document.querySelector(".tschussse-book-pop>div>div>div");
  boxes.forEach((item, idx) => {
    let dataLink = item.getAttribute("data-link");
