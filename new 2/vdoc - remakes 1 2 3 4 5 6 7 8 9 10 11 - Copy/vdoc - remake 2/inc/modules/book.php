...
class Book
{
    ...
    {
        $books = $this->db_handle->get_query("SELECT * FROM ".get_env('TABLE_LINKS')." WHERE MATCH(title) AGAINST('$this->title') LIMIT ".get_env('LINKS_LIMIT')."");
    ...
    {
        $keywords = $this->db_handle->get_query("SELECT * FROM ".get_env('TABLE_KEYWORDS')." WHERE MATCH(keyword) AGAINST('$this->title') LIMIT ".get_env('KEYWORDS_LIMIT')."");
        return $keywords;
    }
}